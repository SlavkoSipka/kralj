...
import {
  ...
  VILLA4_FIRST_FLOOR_APARTMENTS,
  VILLA4_SECOND_FLOOR_APARTMENTS,
  VILLA4_THIRD_FLOOR_APARTMENTS,
  VILLA4_PENTHOUSE_APARTMENTS,
  getFloorName,
} from '../constants/villa4Apartments';
...
const FLOORS = [
  { title: 'Nisko Prizemlje', apartments: VILLA4_GROUND_FLOOR_APARTMENTS },
  { title: 'Visoko Prizemlje', apartments: VILLA4_FIRST_FLOOR_APARTMENTS },
  { title: 'Prvi Sprat', apartments: VILLA4_SECOND_FLOOR_APARTMENTS },
  { title: 'Drugi Sprat', apartments: VILLA4_THIRD_FLOOR_APARTMENTS },
  { title: 'Povučeni Sprat', apartments: VILLA4_PENTHOUSE_APARTMENTS },
];

const villa4ModalImage = (n?: number, fallback?: string) => {
  if (!n) return fallback;
  if ([22, 23, 24, 25, 26, 27].includes(n)) return `/images/vila4/stan ${n}.webp`;
  if ([1, 7, 13, 19].includes(n)) return '/images/vila4/stan 1 - 7 - 13 - 19.webp';
  if ([2, 8, 14, 20].includes(n)) return '/images/vila3/stan 5-11-17.webp';
  if ([3, 9, 15, 21].includes(n)) return '/images/vila4/stan 3 - 9 - 15 - 21.webp';
  return fallback;
};

const Villa4Apartments = () => {
  const [isVisible, setIsVisible] = useState(false);
  const [, setIsLoading] = useState(true);
  const [selectedApartment, setSelectedApartment] = useState<null | { id: number; number: number; size: number; type?: string; image?: string }>(null);
  const [isThankYouOpen, setIsThankYouOpen] = useState(false);
  const { isOpen, open, close } = useModal();
  const { scrolled, showScrollIndicator } = useScroll();
  useIntersectionObserver();
  useSeo({
    title: 'Vila IV – prodaja stanova Vrnjačka Banja | Kralj Residence',
    description:
      'Novogradnja stanova za prodaju u Vili IV, Vrnjačka Banja. Privatni bazen i savremeni sadržaji uz direktnu prodaju od investitora – Kralj Residence.',
    path: '/villa-4',
  });
  ...
  return (
    <div className="relative min-h-screen bg-royal-ivory villa-page overflow-x-hidden">
      ...
      <header className="relative flex min-h-[75vh] items-center">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: 'url("/images/vila3/Kralj k4 (1).webp")' }} />
          ...
          <div className="container-fluid px-6 md:px-10 lg:px-16">
            <div className="max-w-2xl">
              ...
              >
                ...
                <span className="block text-gold">Vila IV</span>
              </h1>
              <p className={`mt-6 max-w-xl text-ts-h6 font-light leading-relaxed text-cream-100/85 transition-all delay-200 duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : '-translate-y-6 opacity-0'}`}>
                Otkrijte ekskluzivnu ponudu stanova u Vili IV. Svaka jedinica je pažljivo dizajnirana
                da pruži savršen spoj luksuza, udobnosti i funkcionalnosti.
              </p>
              <div className={`mt-10 flex flex-wrap items-center gap-4 transition-all delay-300 duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : '-translate-y-6 opacity-0'}`}>
                ...
                <Link to="/royal-aqua" className="btn-royal-outline border-cream-100/40 text-cream-100 hover:bg-cream-100 hover:text-night">
                  Pogledajte Royal Aqua
          ...
          <div className="grid gap-14 lg:grid-cols-2 lg:gap-20">
            <div className="scroll-animate from-left">
              <SectionHeading
                align="left"
                eyebrow="Kontakt"
                title="Zainteresovani za stan u Vili IV?"
      ...
      <ApartmentModal
        ...
        apartment={{
          ...
          image: villa4ModalImage(selectedApartment?.number, selectedApartment?.image || apartment.image) || apartment.image,
          ...
          status: selectedApartment ? apartmentStatusForPath('/villa-4', selectedApartment.number) : apartment.status,
        }}
      />
...
export default Villa4Apartments;
